...
def test_einsum_i_ij_ij(operation, name, device="cpu"):
    # Create sample input tensors
    i, j = 3, 4
    a = np.random.rand(i)  # shape (i,)
    b = np.random.rand(i, j)  # shape (i, j)
    ...
    print(f"{name} einsum_i_ij_ij on {device}:")
    ...
    expected_result = np.einsum("i,ij->ij", a, b)
...
for name, op in [("NumPy", numpy_einsum_i_ij_ij), ("SciPy", scipy_einsum_i_ij_ij)]:
    test_einsum_i_ij_ij(op, name)
...
test_einsum_i_ij_ij(torch_wrapper_cpu(torch_einsum_i_ij_ij), "PyTorch", "cpu")

# PyTorch GPU test
if torch.cuda.is_available():
    test_einsum_i_ij_ij(torch_einsum_i_ij_ij, "PyTorch", "gpu")
else:
