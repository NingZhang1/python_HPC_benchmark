...
def test_qr_performance(sizes, num_runs=5):
    for m, n in sizes:
        ...
        for _ in range(num_runs):
            _, cpu_time, wall_time = measure_time_cpu(numpy_qr, np_matrix)
        ...
        scipy_wall_times = []
        for _ in range(num_runs):
            _, cpu_time, wall_time = measure_time_cpu(scipy_qr, np_matrix)
        ...
        torch_wall_times = []
        for _ in range(num_runs):
            _, cpu_time, wall_time = measure_time_cpu(torch_qr, torch_matrix)
        ...
        if torch.cuda.is_available():
            ...
            torch_gpu_times = []
            for _ in range(num_runs):
                _, gpu_time, _ = measure_time_gpu(torch_qr, torch_matrix_gpu)
